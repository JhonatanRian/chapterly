...
export function IdeasListPage() {
  ...
  const filters: IdeaFilters = {
    ...
    precisa_apresentador: searchParams.get('precisa_apresentador') === 'true' ? true : undefined,
  ...
  const handleVote = async (ideaId: number) => {
    try {
      await ideasService.vote(ideaId);
      toast.success('Voto registrado!');
      refetch();
    } catch (error: any) {
      const message = error.response?.data?.detail || 'Erro ao votar';
      toast.error(message);
    }
  };

  const handleVolunteer = async (ideaId: number) => {
    try {
      await ideasService.volunteer(ideaId);
      toast.success('Você se voluntariou para apresentar esta ideia!');
      refetch();
    } catch (error: any) {
      const message = error.response?.data?.detail || 'Erro ao voluntariar-se';
      toast.error(message);
    }
  };
    ...
    <MainLayout>
      ...
      <div className="mb-4">
        <p className="text-sm text-gray-600 dark:text-gray-400">
          {data?.count || 0} {data?.count === 1 ? 'ideia encontrada' : 'ideias encontradas'}
        </p>
      </div>
